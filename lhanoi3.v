...
Fixpoint lhanoi {n : nat} : configuration 3 n -> configuration _ n -> _ :=
  ...
  | n1.+1 =>
      fun c1 c2 =>
       let p1 := c1 sdisk in
       let p2 := c2 sdisk in
       if p1 == p2 then map (cliftr p2) (lhanoi (cunliftr c1) (cunliftr c2)) else
       let p3 := opeg p1 p2 in
       if lrel p1 p2 then
         map (cliftr p1) (lhanoi (cunliftr c1) (perfect p3)) ++
         map (cliftr p2) (perfect p3 :: lhanoi (perfect p3) (cunliftr c2)) else
         map (cliftr p1) (lhanoi (cunliftr c1) (perfect p2)) ++
         map (cliftr p3) (perfect p2 :: lhanoi (perfect p2) (perfect p1)) ++
         map (cliftr p2) (perfect p1 :: lhanoi (perfect p1) (cunliftr c2))
  end.
...
case: path3SP c1Pcs => // [c1' cs' p1 csE c1'Ecs1'|
  ...
  by rewrite /f csE -{1}[c1]cunliftrK last_map cliftr_sdisk.
...
have [p1Rp| p1NRp] := boolP (lrel p1 p).
  case: (p2 =P p) => [p2Ep|/eqP p2Dp].
    ...
    case: eqP => [cs2E _ | /eqP cs2D Lcs2].
      ...
      rewrite cs2E size_map cliftr_sdisk /=.
  ...
  (* The first disk jumped to an intermediate peg *)
  ...
  case: path3SP c2Pcs2 => // [c2' cs2' p2' cs2E c2'Pcs2' _|
    ...
    by rewrite /f !cliftr_sdisk.
  (* cs has a duplicate *)
  have p4Ep2 : p4 = p2 by rewrite /p4 !cliftr_sdisk.
  have p5Ep1: p5 = p1.
...
case: path3SP c2Pcs2 => // [c2' cs2' p2' cs2E c2'Pcs2' _|
  ...
  case/eqP: p2Dp.
  move: lc1csEp.
  rewrite csE cs2E last_cat /= -[c2]cunliftrK last_map !cliftr_sdisk.
  by move => /(congr1 f); rewrite /f !cliftr_sdisk.
...
have p4Ep2 : p4 = p2 by rewrite [LHS]cliftr_sdisk.
...
have p8Ep : p8 = p by rewrite [LHS]cliftr_sdisk.
have p9Ep2 : p9 = p2.
...
have lc1cs7E : last c1 cs7 = c.
  ...
  rewrite -[c2]cunliftrK !cliftr_sdisk.
...
Lemma gdist_size_lhanoi n (c1 c2 : _ _ n) :
